    ...
    <main className="min-h-screen bg-background flex flex-col relative overflow-hidden text-on-surface font-body">
      ...
      <div className="relative z-10 flex-1 flex flex-col items-center justify-center px-4 w-full h-full pt-12 pb-24">
        ...
        <motion.div 
          ...
          className="w-full max-w-[420px] bg-white/60 backdrop-blur-xl border border-white/80 rounded-[32px] p-8 md:p-10 shadow-[0_20px_40px_rgba(0,0,0,0.05)] relative overflow-hidden"
        >

          <div className="flex flex-col items-center justify-center mb-8">
            <ShieldCheck className="w-10 h-10 text-primary mb-4" />
            <p className="text-center text-on-surface-variant text-xs uppercase tracking-[0.2em] font-bold">
          ...
          <AnimatePresence mode="wait">
            {error && (
              <motion.div 
                ...
                className="mb-8 p-4 bg-error/10 border border-error/20 text-error text-[9px] font-black uppercase tracking-widest text-center rounded-2xl"
              >
                {error}
              </motion.div>
            )}
          ...
          <form className="relative z-10" onSubmit={handleSubmit}>
            <div className="space-y-6">
              ...
              <motion.button 
                ...
                disabled={loading}
                type="submit" 
                className="w-full bg-primary text-[#ffffff] font-h1 text-sm italic py-4 rounded-2xl transition-all flex items-center justify-center gap-3 mt-8 uppercase tracking-[0.2em] shadow-md hover:bg-primary/90 disabled:opacity-60 disabled:cursor-not-allowed"
              >
                {loading ? (
